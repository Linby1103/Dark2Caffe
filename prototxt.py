#feature merge
layer_merge_first=['layer80-conv','layer84-route']
layer_merge_second=['layer92-conv','layer96-route']
...
def save_prototxt(net_info, protofile, region=True):
    ...
    def print_block(block_info, prefix, indent):
        ...
        for key, value in block_info.items():
            ...
            # if value in layer_merge_first:
            #     reoute_layer=True
            #
            # if reoute_layer and value == layer_merge_first[0]:
            ...
            # if value in layer_merge_second:
            #     reoute_layer=True
            #
            #
            # if reoute_layer and value == layer_merge_second[0]:
            #     fp.write('%s    %s: %s \n' % (blanks, key, format_value(value)))
            #
            #
            #     fp.close()
    ...
    props = net_info['props']
    layers = net_info['layers']
    fp.write('name: \"%s\"' % props['name']+'\n')
    fp.write('input: \"%s\"' % props['input']+'\n')
    fp.write('input_dim: %s' % props['input_dim'][0]+'\n')
    fp.write('input_dim: %s' % props['input_dim'][1]+'\n')
    fp.write('input_dim: %s' % props['input_dim'][2]+'\n')
    fp.write('input_dim: %s' % props['input_dim'][3]+'\n')
    fp.write(''+'\n')
    for layer in layers:
